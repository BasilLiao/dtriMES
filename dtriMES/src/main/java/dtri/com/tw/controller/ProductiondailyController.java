...
public class ProductiondailyController {
	...
	public String access(@RequestBody String json_object) {
		...
		// Step2.進行查詢
		resp = dailyService.getData(req.getBody(), req.getPage_batch(), req.getPage_total());
		// Step3.包裝回傳
	...
	public String create(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = dailyService.createData(req.getBody(), user);
		if (check) {
			check = dailyService.save_asData(req.getBody(), user);
		}
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
