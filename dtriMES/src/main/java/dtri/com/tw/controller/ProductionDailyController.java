...
@Controller
public class ProductionDailyController extends AbstractController {
	public ProductionDailyController() {
		super("production_daily.basil");
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = { "/ajax/production_daily.basil" }, method = { RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	public String access(@RequestBody String json_object) {
		...
		// Step2.進行查詢
		check = dailyService.getData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = { "/ajax/production_daily.basil.AR" }, method = { RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = { "/ajax/production_daily.basil.AC" }, method = { RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	public String create(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = dailyService.createData(resp, req, user);
		if (check) {
			check = dailyService.save_asData(resp, req, user);
		}
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
