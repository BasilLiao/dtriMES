...
public class SystemUserController extends AbstractController {
	...
	public String access(@RequestBody String json_object) {
		...
		// Step2.進行查詢
		check = userService.getData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
	...
	public String create(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = userService.createData(resp, req, user);
		if (check) {
			check = userService.save_asData(resp, req, user);
		}
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
	...
	public String modify(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行修改
		check = userService.updateData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
			resp = packageService.setObjResp(resp, req, null);
	...
	public String delete(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行移除
		check = userService.deleteData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
			resp = packageService.setObjResp(resp, req, null);
