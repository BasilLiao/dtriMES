...
import dtri.com.tw.service.OwnUserService;
...
public class OwnUserController extends AbstractController{
	...
	public String access(@RequestBody String json_object) {
		...
		// Step2.進行查詢
		check = ownUserService.getData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = { "/ajax/own_user.basil.AU" }, method = { RequestMethod.PUT }, produces = "application/json;charset=UTF-8")
	public String modify(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = ownUserService.updateData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
			resp = packageService.setObjResp(resp, req, null);
