...
@Controller
public class ProdictionTestController extends AbstractController {
	public ProdictionTestController() {
		super("production_test.basil");// 功能
	}
	...
	public String access(@RequestBody String json_object) {
		...
		// Step2.進行查詢
		check = testService.getData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
	...
	public String create(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = testService.createData(resp, req, user);
		if (check) {
			check = testService.save_asData(resp, req, user);
		}
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
	...
	public String modify(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = testService.updateData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
			resp = packageService.setObjResp(resp, req, null);
	...
	public String delete(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = testService.deleteData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
			resp = packageService.setObjResp(resp, req, null);
