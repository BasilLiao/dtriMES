...
@Controller
public class ProductionSNController extends AbstractController {
	public ProductionSNController() {
		super("production_sn.basil");
	}
	...
	public String access(@RequestBody String json_object) {
		...
		// Step2.進行查詢
		check = snService.getData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
	...
	public String create(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = snService.createData(resp, req, user);
		if (check) {
			check = snService.save_asData(resp, req, user);
		}
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
	...
	public String modify(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = snService.updateData(resp, req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
			resp = packageService.setObjResp(resp, req, null);
	...
	public String delete(@RequestBody String json_object) {
		...
		req = packageService.jsonToObj(new JSONObject(json_object));
		// Step2.進行新增
		check = snService.deleteData(resp,req, user);
		// Step3.進行判定
		if (check) {
			// Step4.包裝回傳
			resp = packageService.setObjResp(resp, req, null);
