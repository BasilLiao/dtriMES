...
public class OqcInspectionFormService {
	...
	public boolean getData(PackageBean bean, PackageBean req, SystemUser user) {
		...
		if (body == null || body.isNull("search")) {
			...
			int ord = 0;
			object_header.put(FFS.ord((ord += 1), FFM.Hmb.H) + "oif_id", FFS.h_t("ID", "50px", FFM.Wri.W_Y));
...
//			obj_m.put(FFS.h_m(FFM.Dno.D_S, FFM.Tag.INP, FFM.Type.TEXT, "", "", FFM.Wri.W_Y, "col-md-1", true, n_val,"sys_note", "備註"));			
			...
			obj_m.put(FFS.h_m(FFM.Dno.D_S, FFM.Tag.INP, FFM.Type.TEXT, "", "", FFM.Wri.W_N, "col-md-2", true, a_val, "oif_f_date", "審核日"));
...
//			}		
				if (oifs == null || oifs.isEmpty()) {
					...
					//	int ord = 0;
						...
						System.out.println(object_oiiitem);
					});
				...
				if (prs.size() > 0) {
					...
					List<ProductionBody> pbs = pbDao.findAllByOldAndPbgid("old", phOrder);
					object_detail.put("oif_q_old", pbs.size());// 轉單old機台數量
					System.out.println("轉單old機台數量: " + pbs.size()); //
					...
					object_details.put("detail", object_detail);
					System.out.println(object_detail);
				}
				...
				object_details.put("oiiitems", object_oiiitems);
				System.out.println(object_details);
			}
			check = true;
	...
	public boolean updateDataCustomized(PackageBean resp, PackageBean req, SystemUser user) {
		...
		try {
			if (oifs.size() > 0) {
				...
				if (title.getString("oif_f_user").length() > 0 && title.getString("oif_f_date") != null) {
