...
@Service
public class PcbaConfigService {
	@Autowired
	...
	public boolean getData(PackageBean bean, PackageBean req, SystemUser user) {
		...
		List<PcbaBody> pcbaBodys = new ArrayList<PcbaBody>();
		...
		if (body == null || body.isNull("search")) {
			...
			//PcbaBody body_one = bodyDao.findAllByPbid(0).get(0);
			// sn關聯表
			int j = 0;
			// Method method;
			for (j = 0; j < 50; j++) {
...
//			obj_m.put(FFS.h_m(FFM.Dno.D_S,FFS.SEL, FFS.TEXT, "", "0", FFM.Wri.W_Y, "col-md-2", true, a_val, "sys_status", "狀態"));
		...
		pcbaBodys = bodyDao.findAllByPbid(0L);
		String pb_value_check = pb_value;
		// 放入包裝(body) [01 是排序][_b__ 是分割直][資料庫欄位名稱]
		JSONArray object_bodys = new JSONArray();
		pcbaBodys.forEach(one -> {
			// sn關聯表
			Method method;
			for (int j = 0; j < 50; j++) {
				int ord = 0;
...
//			check = true;
	...
	public boolean updateData(PackageBean resp, PackageBean req, SystemUser user) {
		...
		try {
			...
			for (Object one : list) {
				// 物件轉換
				PcbaBody p_body = bodyDao.findAllByPbid(0l).get(0);
				...
				if (pb_cell != null && !pb_cell.equals("")) {
					...
					bodyDao.save(p_body);
					// 工作站-項目登入
					ArrayList<PcbaWorkstationItem> items = itemDao.findAllByPcbaWorkstationItem(pb_cell, null);
					if (!pb_value.equals("")) {
						// 登入
						PcbaWorkstationItem one_w = new PcbaWorkstationItem();
						if (items.size() == 1) {
							one_w = items.get(0);
						}
						one_w.setPwipbcell(pb_cell);
						one_w.setPwipbvalue(pb_value);					
						itemDao.save(one_w);
					} else {
						// 移除
